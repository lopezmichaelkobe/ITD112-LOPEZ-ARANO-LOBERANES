...
function UsersTable() {
  ...
  const filteredUsers = data.filter(
    (UserInfo) =>
      ...
      UserInfo.provincecity.toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <table className="table table-borderless datatable">
          <thead className="thead-dark">
            <tr>
              ...
              <th scope="col mx-2">Province or City</th>
            ...
            {currentData.map((UserInfo, index) => {
              return (
                <tr key={index}>
                  ...
                  <td>{UserInfo.provincecity}</td>
                  <td className="">
                  <Link to={`/edituser/${UserInfo._id}`} className="btn btn-outline-success mx-2">Update</Link>
                    <button
